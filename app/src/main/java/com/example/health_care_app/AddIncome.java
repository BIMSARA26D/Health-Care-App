...
public class AddIncome extends AppCompatActivity {
    ...
    private void validate(){
        ...
        if(getId.isEmpty()){
            ...
            String key = mDatabase.child("tblIncome").push().getKey();
            mDatabase.child("tblIncome").child(key).setValue(income_model).addOnSuccessListener(new OnSuccessListener<Void>() {
                @Override
                public void onSuccess(Void unused) {
                    progressDialog.dismiss();
                    Toast.makeText(AddIncome.this, "Successfully Saved!!", Toast.LENGTH_SHORT).show();
                    finish();
                }
                ...
                public void onFailure(Exception e) {
                    ...
                    Toast.makeText(AddIncome.this, "Fail try again.", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    ...
    private void loadDate(){
        ...
        date.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new DatePickerDialog(AddIncome.this,date1,myCalendar.get(Calendar.YEAR),myCalendar.get(Calendar.MONTH),myCalendar.get(Calendar.DAY_OF_MONTH)).show();
            }
        });
